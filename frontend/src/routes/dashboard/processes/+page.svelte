<script lang="ts">
  ...
  import type { ServerListItem, ProcessItem, ProcessAction, ProcessType } from '$lib/api';

  // ── State ────────────────────────────────────────────────────────────────────
  let servers = $state<ServerListItem[]>([]);
  let selectedServer = $state<ServerListItem | null>(null);
  let processes = $state<ProcessItem[] | null>(null);
  ...
  function typeBadge(type: ProcessType): string {
    return type === 'server'
  ...
  async function handleControl(p: ProcessItem, action: ProcessAction) {
    if (!selectedServer) return;
    const key = controlKey(p);
    controlling = new Set([...controlling, key]);
    try {
      await controlProcess(selectedServer.id, p.id, p.type, action);
    } catch (err: any) {
      ...
      error = err.message || `Failed to ${action} process.`;
    } finally {
  ...
  function connectStream(serverId: string) {
    ...
    processes = null;
    const es = createProcessEventSource(serverId);
    es.addEventListener('processes', (event) => {
      try {
        processes = JSON.parse((event as MessageEvent).data);
      } catch { /* ignore */ }
    });
  ...
  async function selectServer(server: ServerListItem) {
    selectedServer = server;
    error = '';
    connectStream(server.id);
  }
...
<header class="fixed top-0 right-0 w-[calc(100%-16rem)] z-40 bg-surface-dim border-b border-outline-variant flex justify-between items-center px-container-padding h-16">
  <div class="flex items-center gap-3">
    ...
    <h1 class="font-headline-md text-headline-md text-on-background m-0">Processes</h1>
    {#if selectedServer}
      <span class="font-label-caps text-[10px] text-on-surface-variant">{selectedServer.name}</span>
    {/if}
  </div>
  <button
    onclick={load}
  ...
  {:else if servers.length === 0}
    <div class="flex flex-col items-center justify-center py-24">
      <div class="w-20 h-20 rounded-2xl bg-surface-container-high border border-outline-variant flex items-center justify-center mb-6">
        <span class="material-symbols-outlined text-[40px] text-on-surface-variant">dns</span>
      </div>
      ...
      <p class="text-body-md text-on-surface-variant max-w-md text-center mb-8">
        Register a PostgreSQL server from the main dashboard to monitor collector processes.
      </p>
    </div>
  {:else}
    <!-- Server selector -->
    <div class="mb-6 flex flex-wrap items-center gap-3">
      ...
      {#each servers as server}
        <button
          ...
          class="cursor-pointer rounded-lg px-3 py-1.5 text-xs font-bold transition-all {selectedServer?.id === server.id ? 'bg-primary text-on-primary' : 'border border-outline-variant bg-surface-container text-on-surface-variant hover:bg-surface-variant'}"
        >
          {server.name}
        </button>
      {/each}
    ...
    <section class="overflow-hidden rounded-lg border border-outline-variant bg-surface-container">
      <div class="flex items-center justify-between border-b border-outline-variant bg-surface-container-low px-4 py-3">
        <div class="flex items-center gap-2">
          ...
          <h2 class="m-0 font-headline-md text-headline-md">Live Processes</h2>
        </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-outline-variant/20">
            ...
            {:else}
              {#each processes as process (process.id + ':' + process.type)}
                {@const key = controlKey(process)}
                {@const busy = controlling.has(key)}
                <tr class="hover:bg-surface-variant/20 transition-colors">
                  <td class="px-4 py-3 font-code-sm text-code-sm text-on-surface font-bold">{process.name}</td>
                  <td class="px-4 py-3">
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border {typeBadge(process.type)}">
                      {process.type}
                    </span>
                  </td>
                  <td class="px-4 py-3 text-sm text-on-surface-variant">
                    {#if process.database_name}
                      {process.database_name}
                    {:else}
                      <span class="text-on-surface-variant">server-wide</span>
                    {/if}
                  </td>
                  <td class="px-4 py-3 font-code-sm text-code-sm text-on-surface">{process.interval}s</td>
                  <td class="px-4 py-3">
                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold border {statusBadge(process.status)}">
                  ...
                  <td class="px-4 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                      {#if process.is_paused}
                        <button
                          onclick={() => handleControl(process, 'resume')}
                          disabled={busy}
                      ...
                      {:else}
                        <button
                          onclick={() => handleControl(process, 'pause')}
                          disabled={busy}
                      ...
                      {/if}
                      <button
                        onclick={() => handleControl(process, 'stop')}
                        disabled={busy}
                      ...
                      <button
                        onclick={() => handleControl(process, 'delete')}
                        disabled={busy}
