<script lang="ts">
  ...
  import { listServers, listProcessHistory } from '$lib/api';
  import type { ServerListItem, ProcessHistoryItem, ProcessType } from '$lib/api';

  // ── State ────────────────────────────────────────────────────────────────────
  let servers = $state<ServerListItem[]>([]);
  let selectedServer = $state<ServerListItem | null>(null);
  let history = $state<ProcessHistoryItem[]>([]);
  ...
  // ── Helpers ──────────────────────────────────────────────────────────────────
  function typeBadge(type: ProcessType | null): string {
  ...
  async function loadServerHistory(server: ServerListItem, resetOffset = true) {
    selectedServer = server;
    if (resetOffset) offset = 0;
    loading = true;
    error = '';
    try {
      history = await listProcessHistory(server.id, limit, offset);
    } catch (err: any) {
      ...
      error = err.message || 'Failed to load process history.';
      history = [];
    } finally {
      loading = false;
    }
  ...
  async function changePage(delta: number) {
    ...
    if (!selectedServer || nextOffset === offset) return;
    offset = nextOffset;
    await loadServerHistory(selectedServer, false);
  }
  ...
  async function load() {
    try {
      ...
      if (servers.length > 0) {
        await loadServerHistory(servers[0]);
      } else {
        history = [];
      }
    } catch (err: any) {
...
<header class="fixed top-0 right-0 w-[calc(100%-16rem)] z-40 bg-surface-dim border-b border-outline-variant flex justify-between items-center px-container-padding h-16">
  <div class="flex items-center gap-3">
    ...
    <h1 class="font-headline-md text-headline-md text-on-background m-0">Process History</h1>
    {#if selectedServer}
  ...
  {:else if servers.length === 0}
    <div class="flex flex-col items-center justify-center py-24">
      ...
      <p class="text-body-md text-on-surface-variant max-w-md text-center mb-8">
        Register a PostgreSQL server from the main dashboard to view collector run history.
      </p>
    </div>
  {:else}
    <!-- Server selector -->
    <div class="mb-6 flex flex-wrap items-center gap-3">
      ...
      {#each servers as server}
        <button
          onclick={() => loadServerHistory(server)}
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-outline-variant/20">
            ...
            {:else if history.length === 0}
              <tr>
                <td colspan="7" class="px-4 py-8 text-center text-on-surface-variant text-sm">
                  No process history found for this server.
                </td>
              </tr>
            {:else}
