<script lang="ts">
  ...
  import {
    listDatabases, getDatabaseStats, getDatabaseUptime,
  ...
  function connectTpsStream(dbId: string) {
    ...
    tpsEventSource.onerror = () => { /* browser auto-reconnects */ };
  }
  ...
  function connectLockStream(dbId: string) {
    ...
    lockEventSource.onerror = () => { /* browser auto-reconnects */ };
  }
  ...
  <div class="glass-panel w-full max-w-lg p-8 shadow-floating">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="font-headline-md text-headline-md font-bold text-on-surface m-0">New Server</h2>
          <p class="text-[11px] text-on-surface-variant mt-0.5">Configure PostgreSQL connection</p>
        </div>
      </div>
      <button onclick={closeServerModal} class="text-on-surface-variant hover:text-error transition-colors">
    ...
    <div class="space-y-4">
      <div>
        <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="srvName">Server Name</label>
        <div class="relative group mt-1.5">
          <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors text-[18px]">badge</span>
          <input bind:value={srvName} class="input-technical text-code-md pl-10 pr-4 py-2.5 w-full" id="srvName" placeholder="Production-01" />
        </div>
      </div>

      <div class="grid grid-cols-3 gap-3">
        <div class="col-span-2">
          <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="srvHost">Host</label>
          <div class="relative group mt-1.5">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors text-[18px]">link</span>
            <input bind:value={srvHost} class="input-technical text-code-md pl-10 pr-4 py-2.5 w-full" id="srvHost" placeholder="db.example.com" />
          </div>
        </div>
        <div>
          <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="srvPort">Port</label>
          <input bind:value={srvPort} class="input-technical text-code-md px-4 py-2.5 w-full mt-1.5" id="srvPort" placeholder="5432" />
        </div>
      </div>

      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="srvUser">Username</label>
          <div class="relative group mt-1.5">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors text-[18px]">person</span>
            <input bind:value={srvUser} class="input-technical text-code-md pl-10 pr-4 py-2.5 w-full" id="srvUser" placeholder="postgres" />
          </div>
        </div>
        <div>
          <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="srvPass">Password</label>
          <div class="relative group mt-1.5">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors text-[18px]">lock</span>
            <input bind:value={srvPass} type="password" class="input-technical text-code-md pl-10 pr-4 py-2.5 w-full" id="srvPass" placeholder="••••••••" />
          </div>
        </div>
      </div>

      <div>
        <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="srvSsl">SSL Mode</label>
        <select bind:value={srvSsl} class="input-technical text-code-md px-4 py-2.5 w-full mt-1.5" id="srvSsl">
      ...
      {#if testResult}
        <div class="bg-{testResult.ok ? 'primary' : 'error'}-container/20 border-l-4 border-{testResult.ok ? 'primary' : 'error'} p-3 rounded-r flex items-start gap-2">
          <span class="material-symbols-outlined text-{testResult.ok ? 'primary' : 'error'} text-[18px]">{testResult.ok ? 'check_circle' : 'error'}</span>
          <p class="font-code-sm text-xs text-{testResult.ok ? 'primary' : 'error'} mt-0.5">{testResult.msg}</p>
        </div>
      {/if}
  ...
  <div class="glass-panel w-full max-w-md p-8 shadow-floating">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="font-headline-md text-headline-md font-bold text-on-surface m-0">Link Database</h2>
          <p class="text-[11px] text-on-surface-variant mt-0.5">Select a server and database name</p>
        </div>
      </div>
      <button onclick={closeDbModal} class="text-on-surface-variant hover:text-error transition-colors">
        <span class="material-symbols-outlined text-[20px]">close</span>
      </button>
    </div>
    ...
    <div class="space-y-4">
      <div>
        <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="dbServer">Server</label>
        <select bind:value={dbServerId} class="input-technical text-code-md px-4 py-2.5 w-full mt-1.5" id="dbServer">
      ...
      <div>
        <label class="text-xs font-bold uppercase tracking-wider text-on-surface-variant" for="dbName">Database Name</label>
        <div class="relative group mt-1.5">
          <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors text-[18px]">storage</span>
          <input bind:value={dbName} class="input-technical text-code-md pl-10 pr-4 py-2.5 w-full" id="dbName" placeholder="postgres" />
        </div>
      </div>

      {#if dbError}
        <div class="bg-error-container/20 border-l-4 border-error p-3 rounded-r flex items-start gap-2">
      ...
      <div class="flex gap-3 pt-2">
        <button onclick={closeDbModal} class="btn-secondary flex-1 py-2.5 font-bold text-sm cursor-pointer">
          Cancel
        </button>
        <button
...
<header class="fixed top-0 right-0 w-[calc(100%-16rem)] z-40 bg-surface-dim border-b border-outline-variant flex justify-between items-center px-container-padding h-16">
  <div class="flex items-center gap-4">
    ...
    <div class="flex items-center gap-3">
      ...
      {#if selectedDb}
        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary/10 text-primary border border-primary/20">
  ...
  <div class="flex items-center gap-6">
    {#if databases.length > 0 && uptime}
      <div class="hidden lg:flex items-center gap-2 px-3 py-1 bg-surface-container rounded-lg border border-outline-variant">
...
<div class="pt-24 px-container-padding pb-12">
  ...
  {:else if databases.length === 0}
    ...
    <div class="flex flex-col items-center justify-center py-24">
      ...
      <p class="text-body-md text-on-surface-variant max-w-md text-center mb-8">
        You haven't linked any PostgreSQL databases yet. Connect a server and link a database to start monitoring.
      </p>
      <button
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-panel-gap mb-panel-gap">
    ...
    <!-- Active Connections -->
    <div class="bg-surface-container border border-outline-variant p-4 flex flex-col gap-1 rounded-lg">
      <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">ACTIVE CONNECTIONS</span>
      <div class="flex items-end justify-between mt-1">
        <span class="font-metric-lg text-metric-lg text-primary font-mono">{sessions ? sessions.length : '--'}</span>
      ...
      <div class="w-full h-1 bg-surface-variant mt-2 rounded-full overflow-hidden">
        <div class="h-full bg-primary rounded-full" style="width: {sessions ? Math.min(sessions.length * 5, 100) : 0}%"></div>
      </div>
    </div>

    <!-- CPU Usage -->
    <div class="bg-surface-container border border-outline-variant p-4 flex flex-col gap-1 rounded-lg">
      <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">CPU USAGE</span>
      <div class="flex items-end justify-between mt-1">
        <span class="font-metric-lg text-metric-lg text-on-surface font-mono">{cpuMetric?.cpu_percent != null ? `${cpuMetric.cpu_percent.toFixed(1)}%` : '--'}</span>
        <span class="text-on-surface-variant text-[10px]">{cpuMetric?.cpu_count ? `${cpuMetric.cpu_count} cores` : '--'}</span>
      </div>
      <div class="w-full h-1 bg-surface-variant mt-2 rounded-full overflow-hidden">
        <div class="h-full bg-primary-container rounded-full" style="width: {cpuMetric?.cpu_percent ?? 0}%"></div>
      </div>
    </div>

    <!-- Memory -->
    <div class="bg-surface-container border border-outline-variant p-4 flex flex-col gap-1 rounded-lg">
      <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">MEMORY</span>
      <div class="flex items-end justify-between mt-1">
        <span class="font-metric-lg text-metric-lg text-on-surface font-mono">{ramMetric?.used_bytes != null ? formatBytes(ramMetric.used_bytes) : '--'}</span>
        <span class="text-on-surface-variant text-[10px]">{ramMetric?.total_bytes != null ? `of ${formatBytes(ramMetric.total_bytes)}` : '--'}</span>
      </div>
      <div class="w-full h-1 bg-surface-variant mt-2 rounded-full overflow-hidden">
        <div class="h-full bg-primary-container rounded-full" style="width: {ramMetric?.percent ?? 0}%"></div>
      </div>
    </div>

    <!-- Transactions/sec -->
    <div class="bg-surface-container border border-outline-variant p-4 flex flex-col gap-1 rounded-lg">
      <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">TRANSACTIONS/SEC</span>
      <div class="flex items-end justify-between mt-1">
        <span class="font-metric-lg text-metric-lg text-tertiary font-mono">{tps.toLocaleString()}</span>
        <span class="text-tertiary text-[10px] font-bold">↑ Peak</span>
      </div>
      <div class="w-full h-1 bg-surface-variant mt-2 rounded-full overflow-hidden">
        <div class="h-full bg-tertiary-container rounded-full" style="width: {Math.min(tps / 3000 * 100, 100)}%"></div>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-12 gap-panel-gap">
        ...
        <table class="w-full table-fixed text-left border-collapse">
          ...
          <thead class="sticky top-0 z-10">
            <tr class="bg-surface-container-high">
              ...
              <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] border-b border-outline-variant">SQL SNIPPET</th>
              <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] border-b border-outline-variant text-center">WAIT</th>
            </tr>
          </thead>
    ...
    <div class="col-span-12 lg:col-span-4 space-y-panel-gap">

      <!-- Live Locks -->
      <div class="bg-surface-container border border-outline-variant flex flex-col rounded-lg overflow-hidden">
        <div class="px-4 py-3 border-b border-outline-variant flex items-center gap-2 bg-surface-container-low">
        ...
        <div class="p-2 space-y-2 max-h-64 overflow-y-auto custom-scrollbar bg-surface-container">
          {#if locks === null}
            <div class="p-3 bg-surface-container-high border-l-4 border-outline flex justify-between items-center rounded-r">
              <div>
                <p class="text-[11px] font-bold text-on-surface-variant uppercase">--</p>
                <p class="text-xs font-code-sm mt-1 text-on-surface-variant">Waiting for collector data</p>
              </div>
              <span class="material-symbols-outlined text-on-surface-variant text-[20px]">hourglass_empty</span>
            </div>
          {:else if locks.length === 0}
            <div class="p-3 bg-surface-container-high border-l-4 border-primary flex justify-between items-center rounded-r">
              <div>
                <p class="text-[11px] font-bold text-primary uppercase">No active locks</p>
                <p class="text-xs font-code-sm mt-1 text-on-surface-variant">Latest collector sample is clear</p>
              </div>
              <span class="material-symbols-outlined text-primary text-[20px]">check_circle</span>
            </div>
          {:else}
            {#each locks as lock}
              <div class="{lock.is_granted === false ? 'bg-error-container/10 border-error' : 'bg-surface-container-high border-outline'} p-3 border-l-4 flex justify-between items-center rounded-r">
                <div class="min-w-0">
                  <p class="{lock.is_granted === false ? 'text-error' : 'text-on-surface-variant'} text-[11px] font-bold uppercase">{lock.mode}</p>
                  <p class="text-xs font-code-sm mt-1 text-on-surface-variant">PID: {lock.holder_pid} -> table #{lock.table_id}</p>
                  {#if lock.query_preview}
                    <p class="text-xs font-code-sm mt-1 text-on-surface truncate max-w-[260px]">{lock.query_preview}</p>
                  {/if}
                </div>
                <span class="{lock.is_granted === false ? 'text-error' : 'text-on-surface-variant'} material-symbols-outlined text-[20px]">{lock.is_granted === false ? 'warning' : 'info'}</span>
              </div>
            {/each}
          {/if}
        </div>
      ...
      <div class="bg-surface-container border border-outline-variant flex flex-col rounded-lg overflow-hidden">
        ...
        <div class="p-4 grid grid-cols-2 gap-4">
          <div class="bg-surface-container-high p-3 rounded-lg text-center border border-outline-variant/30">
        ...
        <div class="px-4 pb-4">
          ...
          <div class="w-full h-1.5 bg-surface-variant rounded-full overflow-hidden">
            <div class="h-full bg-primary rounded-full" style="width: {stats?.index_hit_rate ?? 0}%"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="mt-panel-gap bg-surface-container-lowest border border-outline-variant font-code-sm text-code-sm p-4 overflow-hidden rounded-lg">
    <div class="flex justify-between items-center mb-3">
      <span class="font-label-caps text-on-surface-variant flex items-center gap-2 tracking-widest">
        <span class="w-2 h-2 rounded-full bg-primary status-pulse"></span>
        LIVE POSTGRES LOGS
      </span>
      <span class="text-on-surface-variant opacity-50 text-[10px] font-label-caps">Stream: Active</span>
    </div>
    <div class="space-y-1 opacity-80 text-on-surface">
      <p><span class="text-on-surface-variant">[2023-10-27 14:32:01]</span> <span class="text-primary-container font-bold">LOG:</span> checkpoint starting: time</p>
      <p><span class="text-on-surface-variant">[2023-10-27 14:32:04]</span> <span class="text-primary-container font-bold">LOG:</span> checkpoint complete: wrote 428 buffers (0.1%)</p>
      <p><span class="text-on-surface-variant">[2023-10-27 14:32:15]</span> <span class="text-tertiary font-bold">WARNING:</span> autovacuum worker took too long on "public.audit_log"</p>
      <p><span class="text-on-surface-variant">[2023-10-27 14:32:45]</span> <span class="text-primary-container font-bold">LOG:</span> disconnection: session time: 0:00:12.454 user=admin_root</p>
    </div>
  </div>
...
<button class="fixed bottom-6 right-6 w-14 h-14 bg-primary text-on-primary rounded-full shadow-floating flex items-center justify-center hover:scale-105 active:scale-95 transition-transform group z-50 cursor-pointer">
