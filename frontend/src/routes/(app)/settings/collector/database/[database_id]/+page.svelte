<script lang="ts">
	...
	const selectedDatabase = $derived(databases.find((database) => database.id === selectedDbId) ?? null);
	...
	async function load() {
		try {
			...
			if (!databases.some((database) => database.id === selectedDbId)) {
				if (databases[0]) {
					await goto(`/settings/collector/database/${databases[0].id}`, { replaceState: true });
				}
				return;
			}
	...
	function selectDatabase() {
		if (selectedDbId) goto(`/settings/collector/database/${selectedDbId}`);
	}
	...
	async function updatePaused(config: DatabaseConfigItem, isPaused: boolean) {
		if (!selectedDbId) return;

		try {
			savingPauseId = config.id;
			error = '';
			const updated = await patchDatabaseConfig(selectedDbId, config.id, { is_paused: isPaused });
...
<div class="pt-24 px-container-padding pb-12 space-y-6">
	...
	{:else if databases.length === 0}
		<div class="settings-card px-6 py-12 flex flex-col items-center text-center">
			...
			<p class="text-sm text-on-surface-variant">Connect a database before configuring collectors.</p>
		</div>
