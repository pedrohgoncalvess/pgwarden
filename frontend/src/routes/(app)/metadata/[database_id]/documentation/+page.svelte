<script lang="ts">
	...
	let selectedDatabaseId = $state($page.params.database_id ?? '');
	...
	async function load() {
		try {
			loading = true;
			error = '';
			selectedDatabaseId = $page.params.database_id ?? '';
...
>
	...
	{#if databaseName}
		...
		<span class="font-body-md text-body-md text-on-surface truncate max-w-md">{databaseName}</span>
	{/if}
</header>
