...
<div class="pt-24 px-container-padding pb-12">
  ...
  {:else if servers.length === 0}
    ...
    <div class="flex flex-col items-center justify-center py-24">
      ...
      <h2 class="font-headline-md text-headline-md font-bold text-on-surface mb-2">No servers registered</h2>
  ...
  {:else}
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      <div class="bg-surface-container border border-outline-variant p-4 rounded-lg">
        <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">SERVERS</span>
      ...
      <div class="bg-surface-container border border-outline-variant p-4 rounded-lg">
        <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">DATABASES</span>
        <div class="flex items-end justify-between mt-1">
          <span class="font-metric-lg text-metric-lg text-on-surface font-mono">{databases.length}</span>
          <span class="text-on-surface-variant text-[10px]">monitored</span>
        </div>
      </div>
      <div class="bg-surface-container border border-outline-variant p-4 rounded-lg">
        <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">TOTAL SESSIONS</span>
        <div class="flex items-end justify-between mt-1">
      ...
      <div class="bg-surface-container border border-outline-variant p-4 rounded-lg">
        <span class="text-on-surface-variant font-label-caps text-label-caps tracking-widest">WAITING</span>
        <div class="flex items-end justify-between mt-1">
          <span class="font-metric-lg text-metric-lg text-error font-mono">
            {Object.values(dbSessions).reduce((acc, s) => acc + (s ? s.filter(x => x.wait_event !== null).length : 0), 0)}
          </span>
      ...
      {#each servers as server (server.id)}
        ...
        <div class="server-card overflow-hidden">
          ...
          >
            <div class="flex items-center gap-4">
              <!-- Status Icon -->
              <div class="w-10 h-10 rounded-xl bg-surface-container-high border border-outline-variant/50 flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined {getStatusColor(server.status)} text-[20px]"
              ...
              <div>
                <div class="flex items-center gap-3">
                  <h2 class="font-headline-md text-headline-md font-bold text-on-surface m-0">{server.name}</h2>
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border {getStatusBadge(server.status)} uppercase">
                ...
                <p class="text-[11px] text-on-surface-variant mt-0.5">
                  {serverDbs.length} database{serverDbs.length !== 1 ? 's' : ''} · {totalSessions} session{totalSessions !== 1 ? 's' : ''}
                  {#if totalWaiting > 0}
            ...
            <div class="flex items-center gap-4">
              ...
              {#if totalSessions > 0}
                <div class="hidden sm:flex items-center gap-2">
                  <div class="flex items-center gap-1.5 px-3 py-1 bg-primary/10 border border-primary/20 rounded-full">
                  ...
                  {#if totalWaiting > 0}
                    <div class="flex items-center gap-1.5 px-3 py-1 bg-error/10 border border-error/20 rounded-full">
              ...
              {/if}
              <span class="material-symbols-outlined text-on-surface-variant chevron-anim"
          ...
          {#if isOpen}
            <div class="border-t border-outline-variant/30 px-6 pb-6 pt-4 space-y-4">
              {#if serverDbs.length === 0}
                <p class="text-sm text-on-surface-variant text-center py-4">No databases linked to this server.</p>
              {:else}
                  ...
                  <div class="db-row overflow-hidden">
                    ...
                    <div class="flex items-center justify-between px-4 py-3 border-b border-outline-variant/20">
                      <div class="flex items-center gap-3">
                        ...
                        <span class="font-code-sm text-code-sm font-bold text-on-surface">{db.name}</span>
                        {#if !db.status}
                          <span class="text-[10px] px-1.5 py-0.5 bg-error/10 text-error border border-error/20 rounded font-bold uppercase">inactive</span>
                        {/if}
                      </div>
                      <div class="flex items-center gap-3">
                        {#if sessions === null}
                          <span class="text-[10px] text-on-surface-variant animate-pulse">connecting…</span>
                        {:else}
                          <span class="text-[10px] text-on-surface-variant">
                            <span class="text-primary font-bold">{sCount}</span> total
                            · <span class="text-tertiary font-bold">{aCount}</span> active
                            {#if wCount > 0}· <span class="text-error font-bold">{wCount}</span> waiting{/if}
                          </span>
                        {/if}
                        <div class="w-1.5 h-1.5 rounded-full {sessions === null ? 'bg-tertiary animate-pulse' : sCount > 0 ? 'bg-primary pulse-dot' : 'bg-outline'}"></div>
                      </div>
                    </div>
                      ...
                      <table class="w-full text-left border-collapse">
                        <thead>
                          <tr class="bg-surface-container-lowest/60">
                            <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] tracking-widest">PID</th>
                            <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] tracking-widest">USER</th>
                            <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] tracking-widest">STATE</th>
                            <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] tracking-widest">DURATION</th>
                            <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] tracking-widest">QUERY</th>
                            <th class="px-4 py-2 font-label-caps text-on-surface-variant text-[10px] tracking-widest text-center">WAIT</th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-outline-variant/20">
                          ...
                          {:else if sessions.length === 0}
                            <tr>
                              <td colspan="6" class="px-4 py-5 text-center text-on-surface-variant text-sm">
                          ...
                          {:else}
                            {#each sessions as session (session.pid)}
                              <tr class="session-row">
                                <td class="px-4 py-2.5 font-code-sm text-code-sm text-on-surface-variant">{session.pid}</td>
                                <td class="px-4 py-2.5 font-code-sm text-code-sm text-on-surface">{session.user_name || '--'}</td>
                                <td class="px-4 py-2.5">
                                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold {getStateChip(session.state)}">
                                    {session.state || '--'}
                                  </span>
                                </td>
                                <td class="px-4 py-2.5 font-code-sm text-code-sm {session.state === 'active' ? 'text-tertiary font-bold' : 'text-on-surface-variant'}">
                                  {formatDuration(session.query_start)}
                                </td>
                                <td class="px-4 py-2.5 max-w-xs">
                                  <code class="font-code-sm text-code-sm text-on-surface block truncate max-w-[300px]">
                                    {session.query_preview || session.application_name || '--'}
                                  </code>
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                  {#if session.wait_event}
                                    <span class="material-symbols-outlined text-error text-[16px]" title="{session.wait_event_type}: {session.wait_event}">block</span>
                                  {:else}
