<script lang="ts">
	...
	import { listDatabases, listServers, getIndexAnalytics, formatBytes, formatNumber } from '$lib/api';
	import type {
		DatabaseListItem,
	...
	function analyticsHref(databaseId: string): string {
		...
		return `/analytics/${databaseId}/index${queryString}`;
	}
	...
	let searchedIndexes = $derived.by(() => {
		...
		return list.filter(
			(idx) =>
				...
				displayTableName({ schema_name: idx.schema_name, table_name: idx.table_name })
					.toLowerCase()
					.includes(term)
		);
	});
	...
	{:else}
			...
			<div class="flex flex-wrap items-end gap-4">
				...
				<div class="flex flex-col gap-2 min-w-[16rem] relative">
					...
					>
						...
						<span
							...
							style="transform: rotate({tableDropdownOpen ? 180 : 0}deg)">expand_more</span
						>
					</button>

					{#if tableDropdownOpen}
						<div
							class="absolute top-full left-0 right-0 mt-1 z-50 rounded-lg border border-outline-variant bg-surface-container shadow-lg overflow-hidden"
						>
							...
							<div class="max-h-48 overflow-y-auto p-1">
								{#each searchedTables as table}
									<label
										class="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer hover:bg-surface-variant text-xs text-on-surface"
									>
										<input
											type="checkbox"
				...
				<div class="flex flex-col gap-2 min-w-[16rem] relative">
					...
					>
						...
						<span
							class="material-symbols-outlined text-[14px] transition-transform duration-200"
							style="transform: rotate({indexDropdownOpen ? 180 : 0}deg)">expand_more</span
						>
					</button>
			...
			{#if selectedTableIds.length > 0 || selectedIndexIds.length > 0}
				<div class="mt-3 flex flex-wrap items-center gap-2">
					{#each selectedTableIds as tableId}
						...
						{#if table}
							<span
								class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold border border-outline-variant bg-surface-container text-on-surface"
							>
					...
					{#each selectedIndexIds as indexId}
						...
						{#if idx}
							<span
								class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold border border-primary/30 bg-primary/10 text-primary"
							>
								{idx.index_name}
								<button
									onclick={() => {
			...
			<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					...
					<p class="font-headline-md text-headline-md font-bold text-on-surface">
						{formatNumber(data.kpis.total_indexes)}
					</p>
				</div>
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					<p class="text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">
					...
					<p class="font-headline-md text-headline-md font-bold text-on-surface">
						{formatBytes(data.kpis.total_size_bytes)}
					</p>
				</div>
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					<p class="text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">
					...
					<p class="font-headline-md text-headline-md font-bold text-on-surface">
						{formatPercent(data.kpis.avg_hit_rate)}
					</p>
				</div>
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					<p class="text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">
					...
					<p class="font-headline-md text-headline-md font-bold text-on-surface">
						{formatNumber(data.kpis.avg_scan_qt)}
					</p>
				</div>
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					<p class="text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">
					...
					<p class="font-headline-md text-headline-md font-bold text-on-surface">
						{formatNumber(data.kpis.unused_indexes)}
					</p>
				</div>
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					<p class="text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">
					...
					<p class="font-headline-md text-headline-md font-bold text-on-surface">
						{data.kpis.unique_indexes} / {data.kpis.primary_indexes}
					</p>
				</div>
			</div>
		{/if}
		...
		>
			...
			>
				...
				<div class="relative">
					...
					{#if chartHelpOpen}
						<div
							class="absolute right-0 top-full mt-2 w-72 z-50 rounded-lg border border-outline-variant bg-surface-container-high shadow-lg p-3 text-xs text-on-surface"
						>
							...
							<ul class="space-y-1.5 text-on-surface-variant list-disc pl-4">
								<li>
									<span class="font-medium text-primary">Solid line</span> — total size of all selected
									indexes (left axis).
								</li>
								<li>
									<span class="font-medium text-amber-500">Dashed line</span> — average buffer hit rate
									across selected indexes (right axis, 0–100%).
								</li>
								<li>
									<span class="font-medium text-on-surface">Hover</span> — also shows total index scans
					...
					>
						...
						{#if data.timeline.length > 0}
							{#each [0, Math.floor((data.timeline.length - 1) / 2), data.timeline.length - 1] as idx}
								...
								<text
									{x}
									y={timelineChartHeight - 8}
									text-anchor="middle"
									class="text-[10px] fill-on-surface-variant"
									>{formatChartDate(data.timeline[idx].collected_at)}</text
								>
							{/each}
						{/if}
						...
						{#if timelineSizePoints.length > 0}
							...
							<text
								...
								y={padding.top + 4}
								text-anchor="end"
								class="text-[10px] fill-on-surface-variant">{formatBytes(maxSize)}</text
							>
							<text
								x={padding.left - 8}
								y={timelineChartHeight - padding.bottom}
								text-anchor="end"
								class="text-[10px] fill-on-surface-variant">0 B</text
							>
						{/if}
						...
						<text
							...
							y={padding.top + 4}
							text-anchor="start"
							class="text-[10px] fill-amber-500">100%</text
						>
						<text
							x={timelineChartWidth - padding.right + 8}
							y={timelineChartHeight - padding.bottom}
					...
					{#if timelineHoverIndex !== null}
						...
						>
							...
							<p class="text-on-surface-variant">
								Total size: <span class="font-bold text-on-surface"
									>{formatBytes(point.total_size_bytes)}</span
								>
							</p>
							<p class="text-on-surface-variant">
								Total scans: <span class="font-bold text-on-surface"
									>{formatNumber(point.total_scans)}</span
								>
							</p>
							<p class="text-on-surface-variant">
								Avg hit rate: <span class="font-bold text-on-surface"
									>{formatPercent(point.avg_hit_rate)}</span
								>
							</p>
						</div>
					{/if}
		...
		<section class="rounded-lg border border-outline-variant bg-surface-container overflow-hidden">
						...
						<tbody class="divide-y divide-outline-variant">
							{#each paginatedItems as item}
								<tr class="hover:bg-surface-variant/50 transition-colors">
									...
									<td class="px-4 py-3">
										<span
											class="inline-flex px-2 py-0.5 rounded text-[10px] font-bold border {badgeClass(
												item.index_type
											)}"
										>
											{item.index_type}
									...
									<td class="px-4 py-3 text-on-surface font-medium"
										>{formatBytes(item.latest_size_bytes)}</td
									>
									...
									<td class="px-4 py-3">
										<div class="flex gap-1">
											...
											{#if item.is_unique}
												<span
													class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-secondary/10 text-secondary border border-secondary/20"
													>UQ</span
												>
											{/if}
											{#if !item.latest_scan_qt}
												<span
													class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-error/10 text-error border border-error/20"
													>unused</span
												>
											{/if}
										</div>
									...
									<td class="px-4 py-3">
										<button
											onclick={() =>
												(expandedIndex = expandedIndex === item.index_id ? null : item.index_id)}
											class="text-on-surface-variant hover:text-primary transition-colors cursor-pointer"
										>
											<span class="material-symbols-outlined text-[18px]"
												>{expandedIndex === item.index_id ? 'expand_less' : 'expand_more'}</span
											>
										</button>
									</td>
								</tr>
										...
										<td colspan="8" class="px-4 py-4">
											<div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs mb-4">
												...
												<div>
													...
													<p class="text-on-surface font-medium">
														{formatNumber(item.latest_tup_read_qt)}
													</p>
												</div>
												<div>
													<p class="text-on-surface-variant mb-0.5">Tuples fetched</p>
													<p class="text-on-surface font-medium">
														{formatNumber(item.latest_tup_fetch_qt)}
													</p>
												</div>
											</div>
											{#if item.history.length > 1}
												...
												<div class="h-32 w-full">
													<svg width="100%" height="100%" class="overflow-visible">
														{#each item.history as point, i}
															{@const hx =
																...
																(item.history.length <= 1
																	? 0
																	: (i / (item.history.length - 1)) * usableW)}
															{@const hy =
																histPadding.top + usableH - (point.size_bytes / maxSize) * usableH}
															...
															{#if i > 0}
																...
																{@const px =
																	...
																	(item.history.length <= 1
																		? 0
																		: ((i - 1) / (item.history.length - 1)) * usableW)}
																{@const py =
																	histPadding.top + usableH - (prev.size_bytes / maxSize) * usableH}
																<line
																	x1={px}
																	y1={py}
																	x2={hx}
														...
														{/each}
														<text
															x={histPadding.left - 8}
															y={histPadding.top + 4}
															text-anchor="end"
															class="text-[9px] fill-on-surface-variant"
															>{formatBytes(maxSize)}</text
														>
														<text
															x={histPadding.left - 8}
															y={chartH - histPadding.bottom}
															text-anchor="end"
															class="text-[9px] fill-on-surface-variant">0</text
														>
													</svg>
												</div>
											{/if}
			...
			{#if sortedItems.length > tablePageLimit}
				<div
					class="flex items-center justify-between border-t border-outline-variant bg-surface-container-low px-4 py-3"
				>
					<span class="text-xs text-on-surface-variant">
						Showing {tablePageOffset + 1}-{Math.min(
							tablePageOffset + tablePageLimit,
							sortedItems.length
