<script lang="ts">
	...
	let timestamps = $derived(
		data
			? allTimestamps(
					data.database_size_history,
					data.table_size_history,
					selectedTableIds,
					showDatabase
				)
			: []
	...
	let minBytesValue = $derived.by(() => {
		if (!data || timestamps.length === 0) return 1;
		let min = Infinity;
		visibleDbValues.forEach((v) => {
			if (v > 0) min = Math.min(min, v);
		});
		visibleTableValues.forEach((tv) =>
			tv.values.forEach((v) => {
				if (v > 0) min = Math.min(min, v);
			})
		);
	...
	let tableMap = $derived.by(() => {
		...
		return new Map(
			currentData.tables.map((t) => [t.id, { schema_name: t.schema_name, table_name: t.name }])
		);
	});
			...
			<div class="flex flex-wrap items-end gap-4">
				...
				<div class="flex flex-col gap-2">
					...
					<div class="flex items-center gap-4">
						...
						<label class="flex items-center gap-1.5 text-xs text-on-surface cursor-pointer">
							<input type="checkbox" bind:checked={logScale} class="accent-primary w-3.5 h-3.5" />
							Log scale
						</label>
					</div>
				</div>
					...
					{#each selectedTableIds as tableId, i}
						...
						{#if table}
							<span
								...
								style="border-color: {colors[(i + 1) % colors.length]}; background-color: {colors[
									(i + 1) % colors.length
								]}1A; color: {colors[(i + 1) % colors.length]};"
							>
				...
				<div class="flex items-center gap-4 text-xs">
					...
					{#each selectedTableIds as tableId, i}
						...
						{#if table}
							<span class="flex items-center gap-1.5">
								<span
									class="w-3 h-1 rounded bg-current"
					...
					>
						...
						{#each generateYTicks() as tick}
							...
							<text
								...
								y={tick.y + 4}
								text-anchor="end"
								class="text-[10px] fill-on-surface-variant">{tick.label}</text
							>
						{/each}
						...
						{#if timestamps.length > 0}
							{#each [0, Math.floor((timestamps.length - 1) / 2), timestamps.length - 1] as idx}
								...
								<text
									...
									>{formatChartDate(timestamps[idx])}</text
								>
							{/each}
						{/if}
						...
						>
							...
							{#if tooltip.database}
								<p class="text-on-surface-variant">
									Database: <span class="font-bold text-on-surface"
										>{formatBytes(tooltip.database.size_bytes)}</span
									>
								</p>
							{/if}
							{#each tooltip.tables as t}
								<p class="text-on-surface-variant">
									{displayTableName(t)}:
									<span class="font-bold text-on-surface">{formatBytes(t.size_bytes)}</span>
								</p>
							{/each}
						</div>
					{/if}
