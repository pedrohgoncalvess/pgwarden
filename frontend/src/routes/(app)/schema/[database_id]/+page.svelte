...
<div class="px-container-padding pb-10 pt-24">
  ...
  {:else if databases.length === 0}
    <div class="flex flex-col items-center justify-center py-24">
      ...
      <a href="/overview" class="flex cursor-pointer items-center gap-2 rounded-lg bg-primary px-8 py-3 text-sm font-bold text-on-primary">
        ...
        <div bind:this={schemaScrollEl} bind:clientWidth={boardViewportWidth} class="schema-scroll custom-scrollbar" onwheel={handleWheel}>
          <div class="schema-canvas" style={`width: ${boardWidth}px; height: ${canvasSize.height * zoom}px;`}>
            ...
            {#each visibleTables as table (table.id)}
              ...
              <article
                class="schema-table {dragState?.tableId === table.id ? 'dragging' : ''} {selectedSchema && table.schema_name === selectedSchema ? 'schema-selected' : ''} {selectedTableId === table.id ? 'table-active' : ''}"
                ...
                onkeydown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); handleTableClick(table.id); } }}
  ...
  >
    <div class="flex items-center justify-between border-b border-outline-variant bg-surface-container px-4 py-3">
      <div class="min-w-0 flex-1">
        ...
        <h2 id="sidebar-title" class="truncate font-headline-md text-headline-md font-bold text-on-surface">
          {selectedTable.name}
        </h2>
      </div>
      <button
    ...
    <div class="custom-scrollbar flex-1 overflow-y-auto p-4">
      ...
      {:else if historyItems.length === 0}
        <div class="flex flex-col items-center justify-center py-12 text-center">
          <span class="material-symbols-outlined mb-2 text-[32px] text-on-surface-variant">history_toggle_off</span>
          ...
          {#each historyItems as item (item.id)}
            <li class="rounded-lg border border-outline-variant/50 bg-surface-container p-3">
              <div class="mb-1 flex items-center gap-2">
                ...
                <span class="text-[10px] font-label-caps uppercase tracking-wider text-on-surface-variant">
                  {item.type}
                </span>
                <span class="rounded px-1.5 py-0.5 text-[9px] font-bold uppercase {historyActionClass(item.action)}">
                  {item.action}
                </span>
              </div>
