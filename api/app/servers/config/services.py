...
async def _get_runs_by_server(server_id: str):
    ...
    async with DatabaseConnection() as conn:
        ...
        db_result = await conn.execute(
            select(Database.id, Database.db_name).where(Database.server_id == internal_id)
        )
        databases = {row[0]: row[1] for row in db_result.all()}

        db_configs = []
        if databases:
            db_cfg_result = await conn.execute(
                select(ConfigDatabase).where(ConfigDatabase.database_id.in_(databases.keys()))
            )
    ...
    for cfg in db_configs:
        runs.append({
            ...
            "database_name": databases.get(cfg.database_id),
            "name": cfg.name,
            "type": "database",
            "interval": cfg.interval,
            "is_paused": cfg.is_paused,
    ...
    async with DatabaseConnection() as conn:
        ...
        if databases:
            db_cfg_result = await conn.execute(
                ...
                .where(ConfigDatabase.database_id.in_(databases.keys()))
            )
        ...
        for cfg_id, db_id in db_config_rows:
            config_lookup[cfg_id] = {
                "type": "database",
                "name": None,
                "database_id": db_id,
                "database_name": databases.get(db_id),
            }

        filters = []
        if server_config_ids:
            filters.append(Run.config_server_id.in_(server_config_ids))
        if db_config_ids:
            filters.append(Run.config_database_id.in_(db_config_ids))
