...
from app.databases.docs.models import ColumnDocPut
from app.databases.docs.services.common import resolve_object
from app.databases.docs.services.embeddings import build_column_embedding_text
from database.models.doc.column import ColumnDoc
from database.models.doc.object_tag import ColumnDocTag
...
async def put_column_doc(db: AsyncSession, database_id: UUID, column_id: int, doc_in: ColumnDocPut, user_id: int) -> ColumnDoc:
    ...
    table = await _load_column_table(db, obj.table_id)
    doc_or_data = existing_doc or ColumnDoc(column_id=obj.id, **doc_in.model_dump())
    embedding_text = build_column_embedding_text(obj, table, doc_or_data)
...
async def _load_column_table(db: AsyncSession, table_id: int) -> Table:
...
async def get_column_doc(db: AsyncSession, database_id: UUID, column_id: int) -> dict:
    ...
    tags_query = select(Tag).join(ColumnDocTag).filter(ColumnDocTag.column_doc_id == doc.id)
